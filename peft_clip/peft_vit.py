...
from peft_clip.peft_module import VPT, MaskedLinear, LoRA, SSF, AdapterBlock
...
class ViT_Tuner(nn.Module):
    ...
    def __init__(self, cfg, vit_model, num_classes):
        ...
        if isinstance(vit_model, CLIP_ViT):
            ...
            raise ValueError(f"Unsupported backbone: {vit_model.__class__.__name__}")
        ...
        if (use_adapter or use_lora or use_lora_mlp) and (adapter_dim is None):
        ...
        if use_adapter:
            adapter_list = nn.ModuleList([
                *[None] * _start,
                *[AdapterBlock(in_dim=emb_dim, bottle_dim=adapter_dim, dtype=dtype, use_scale=cfg.adapter_scale) for _ in
                  range(_start, _end)],
                *[None] * (n_layers - _end)
            ])
        else:
        ...
        if use_lora_mlp:
            lora_mlp_list = nn.ModuleList([
                *[None] * _start,
                *[nn.ModuleDict({
                    "1": LoRA(in_dim=emb_dim, bottle_dim=adapter_dim, dtype=dtype),
                    "2": LoRA(in_dim=mlp_in_dim, bottle_dim=adapter_dim, dtype=dtype),
                }) for _ in range(_start, _end)],
                *[None] * (n_layers - _end)
            ])
        else:
...
class Peft_ViT(nn.Module):
    def __init__(self, vit_model):
        super().__init__()
        assert isinstance(vit_model, (CLIP_ViT, ViT)), "Unsupported"
        if isinstance(vit_model, CLIP_ViT):
    ...
    def forward(self, x, tuner=None, head=None) -> torch.Tensor:
        x = x.to(self.dtype)
        ...
        for i in range(n_layers):
            ...
            if tuner is not None:
                ...
                vpt = adapter = lora = lora_mlp = ssf_attn = ssf_mlp = ssf_ln = masked_linear = None

            x = vpt(x) if vpt is not None else x
        ...
        return head(x) if head is not None else x
