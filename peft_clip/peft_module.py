...
class AdapterBlock(nn.Module):
    def __init__(self, in_dim, bottle_dim, dtype=None, use_scale=False):
        super().__init__()
...
class MaskedLinear(nn.Module):
    def __init__(self, weight, bias, ratio=0.0, generator=None):
        ...
        masked_index = torch.randperm(num_params, generator=generator)[:num_masked]
        mask = torch.zeros(num_params, dtype=bool).scatter(dim=0, index=masked_index, value=True)
